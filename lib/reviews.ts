...
const fetchPlaceReviews = async (pin: Pin): Promise<Review[]> => {
  try {
    logger.log('Fetching reviews for pin:', pin.title, 'URL:', pin.google_maps_url)
    
    if (!pin.google_maps_url) {
      logger.log('No Google Maps URL, skipping')
      return []
    }
    ...
    if (reviewsResponse.ok) {
      ...
      if (reviewsData.reviews && reviewsData.reviews.length > 0) {
        ...
        logger.log('Transformed reviews:', transformedReviews)
        return transformedReviews
      } else {
        logger.log('No reviews found for this place')
        return []
      }
    } else {
      logger.log('Failed to fetch reviews, status:', reviewsResponse.status)
      return []
    }
  } catch (error) {
    logger.warn('Error fetching reviews for place:', pin.title, error)
    return []
  }
}
...
export const fetchPackReviews = async (pins: Pin[]): Promise<Review[]> => {
  try {
    logger.log('Fetching Google Maps reviews for pins:', pins.length)
    ...
    logger.log('Places with URLs:', placesWithUrls.length)
    ...
    logger.log('Total reviews collected:', allReviews.length)
    
    // If no reviews were collected, log why
    if (allReviews.length === 0) {
      logger.log('No Google Maps reviews collected - this is normal if:')
      logger.log('1. Places don\'t have proper Google Maps URLs')
      logger.log('2. Google Places API returns no reviews for the places')
      logger.log('3. Place details cannot be found')
      logger.log('Falling back to local experience reviews...')
    }
    
    return allReviews.slice(0, 6) // Return max 6 reviews
  } catch (error) {
    logger.error('Error fetching Google Maps reviews:', error)
    return []
  }
}
...
export const aggregateAndStorePackReviews = async (packId: string, pins: Pin[], supabase: any): Promise<void> => {
  try {
    ...
    if (error) {
      logger.error('Error storing reviews:', error)
      throw error
    }
    
    logger.log(`Successfully stored ${finalReviews.length} reviews for pack ${packId} (source: ${reviewsSource})`)
    
  } catch (error) {
    logger.error('Error aggregating and storing pack reviews:', error)
    throw error
  }
}
...
export const refreshPackReviews = async (packId: string, pins: Pin[], supabase: any): Promise<void> => {
  try {
    logger.log('Refreshing reviews for pack:', packId)
    await aggregateAndStorePackReviews(packId, pins, supabase)
  } catch (error) {
    logger.error('Error refreshing pack reviews:', error)
    throw error
  }
} 